...
const loadImageData = async (input: File) => {
...
const handleConvert = async () => {
  ...
  try {
    const imageData = await loadImageData(file.value);
    ...
    errorMsg.value = err?.message || 'Gagal convert ke AVIF.';
  } finally {
  ...
  <div class="w-full max-w-xl mx-auto mt-6" v-cloak>
    <div
      ...
      <div class="mb-8 text-center relative z-10">
        ...
        <p class="text-gray-500 text-[15px] leading-relaxed max-w-sm mx-auto">Optimize your PNG or JPG images by
          converting them to AVIF directly in the browser.</p>
      </div>
      ...
      <form v-if="!resultBlobUrl" @submit.prevent="handleConvert" class="flex flex-col gap-6 relative z-10">
        ...
        <div class="relative group">
          <label for="image-file"
            ...
            <div v-if="!previewUrl"
              ...
              <UploadCloud class="w-12 h-12 mb-4 text-gray-400 group-hover:text-emerald-500 transition-colors"
                stroke-width="1.5" />
            ...
            <input id="image-file" type="file" accept="image/png, image/jpeg, image/webp" class="hidden"
        ...
        <button type="submit" :disabled="isConverting || !file"
          class="relative overflow-hidden w-full px-5 py-4 bg-gray-900 text-white font-semibold rounded-2xl hover:bg-gray-800 focus:outline-none focus:ring-4 focus:ring-gray-200 transition-all duration-300 shadow-xl shadow-gray-900/20 disabled:opacity-70 disabled:cursor-not-allowed group">
      ...
      <div v-else
        ...
        <p class="text-gray-500 text-sm mb-6 text-center max-w-xs">Your image has been optimized into next-gen AVIF
          format.</p>
